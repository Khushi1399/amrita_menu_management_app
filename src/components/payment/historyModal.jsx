...
const PaymentMain = (props) => {
    ...
    props.items.map(item =>
        amount = amount + item.itemQuantity * item.itemPrice
    )

    return (
        <div className="text-sm text-gray-500 shadow-md pl-10 mt-5 w-90">
            <h2 className="block uppercase tracking-wide text-pink-400 text-2xl font-bold mb-4">{props.date}</h2>
            {console.log(props.items)}
            {
                ...
                ) : <label className='text-lg text-bold font-medium leading-6 text-gray-900'>You have not ordered any items.</label>
            }
            {
                props.items.length > 0 && <label className='text-lg font-bold font-medium leading-6 text-pink-900'>Amount :
                    <label className='text-lg text-bold font-medium leading-6 text-gray-900'>{amount}</label>
                </label>
            }
        </div>
    )
